...
prev_transfers_tb as (
        ...
        FROM (
                ...
                FROM transfers WHERE updated_at::date = current_timestamp::date AND status = 'pending') transfer_out,
        LATERAL (
                SELECT user_id, updated_at, updated_at::date - lag(updated_at::date) over (order by updated_at) as diff, transfer_amount_attempted 
                FROM transfers WHERE end_date <> 'infinity' and transfers.user_id = transfer_out.user_id order by updated_at desc limit 3) transfers_in
