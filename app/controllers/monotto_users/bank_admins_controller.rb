class MonottoUsers::BankAdminsController < MonottoUsers::ApplicationController
   ...
   def update
      @bank_admin = BankAdmin.find_by(id: params[:id]).try{ |obj| obj.update_attributes(bank_admin_params)}
   ...
   def bank_admin_params
     params.require(:bank_admin).permit(:financial_institution_id, :email, :name, :title, :phone, :notes, :is_primary, :password_digest)
   end
end
